...
class Lab6 extends GLJPanel implements GLEventListener  {
	...
	public void display(GLAutoDrawable drawable) {
		...
		PyramidBaseOnCircle(gl2,12);


	} // end display()

	public void PyramidBaseOnCircle(GL2 gl2,int numVertices){
		double angleIncrement = 2 * Math.PI / numVertices; // zwiększanie kąta do budowy piramidy
		angle = 0; // początkowy kąt
		for (int i = 1; i <= numVertices; i++)  // pętla powielająca wielokąty
		{
			gl2.glBegin(GL2.GL_POLYGON); // instrukcja do rozpoczęcia tworzenia trójkątów
			x = (float) (radius * Math.cos(angle));
			z = (float) (radius * Math.sin(angle));

			gl2.glMaterialfv (GL2.GL_FRONT, GL2.GL_AMBIENT_AND_DIFFUSE, materials[7],8); // nadanie trójkątom materiału w kolorze brązu

			gl2.glVertex3f( x, -2.0f, z ); // generowanie trójkątów
			gl2.glVertex3f( 0.0f, 4.0f, 0.0f ); // pozycja stożka w przestrzeni 3D, gdzie v1 to wysokość (y)
	...
	public void init(GLAutoDrawable drawable) {
		...
		float[] GL_AMBIENT_AND_DIFFUSE = { 0.8f, 0.8f, 0.8f, 1f }; // ustawienie kolorów na neutralne
		gl2.glLightModelfv(GL2.GL_LIGHT_MODEL_AMBIENT, GL_AMBIENT_AND_DIFFUSE,0 ); // funkcja do ustawienia kolor´ow materialu otoczenia, rozproszonego, lustrzanego i emisyjnego, określamy kolejno stronę, rodzaj
