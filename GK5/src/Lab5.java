...
class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
    ...
    private void draw(GL2 gl2) {
        ...
        // range -5 to 5.)
        if (objectNumber ==1) {             //korkociąg

            Korkociag(gl2,1,12);

        }
        else if (objectNumber == 2){        //piramida
            Pyramid(gl2,4,12); // object, size of object, number of triangles
        }

    }
    public void Korkociag(GL2 gl2,int radius, int numberOfTurns){
        ...
        for (int i = 1; i <= numVertices; i++) {
            ...
            gl2.glColor3f(0.5f,0.0f,0.0f);
            x = (float) (radius * Math.cos(angle-5));
            z = (float) (radius * Math.sin(angle-10));
    ...
    public void Pyramid(GL2 gl2,float size, float n)
    {
        ...
        for (int i = 1; i <= n; i++) {
            gl2.glColor3f(139, 0+i, 0); // setting colour to DarkRed
